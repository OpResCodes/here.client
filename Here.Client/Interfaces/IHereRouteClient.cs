...
namespace Here.Client.Interfaces
{
    public interface IHereRouteClient : IHereBaseClient
    {
        ...
        [Get("7.2/calculateroute.json?waypoint0=geo!{originGeocode}&waypoint1=geo!{destinationGeocode}&metricSystem={units}&mode={mode};{vehicle};traffic:{traffic}&routeAttributes={routeAttributes}&app_id={appId}&app_code={appCode}")]
        Task<RouteResponse> GetRouteByAddressAsync([Path] string originGeocode, [Path] string destinationGeocode, [Path] string units, [Path] string mode, [Path] string vehicle, [Path] string traffic, [Path] string routeAttributes);
        ...
        //[Get("7.2/calculateisoline.json?start={start}&rangetype={rangetype}&range={range}&mode={mode};{vehicle};traffic:{traffic}&app_id={appId}&app_code={appCode}")]
